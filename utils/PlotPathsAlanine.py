...
def PlotPathsAlanine(paths, paths_forward, target, filename, paths_linear=None):
    colors = []
    ...
    paths_ = paths.reshape(paths.shape[0], -1, 22, 3).detach().cpu().numpy()
    paths_forward_ = paths_forward.reshape(paths.shape[0], -1, 22, 3).detach().cpu().numpy()
    if paths_linear is not None:
        paths_linear_ = paths_linear.reshape(paths.shape[0], -1, 22, 3).detach().cpu().numpy()
    ...
    for i in range(0, paths_.shape[0]):
        ...
        psis_start.append((new_dihedral(paths_forward_[i, 0, angle_1, :])))
        phis_start.append(new_dihedral(paths_forward_[i, 0, angle_2, :]))

        psi = []
        phi = []
        for j in range(0, paths_forward_.shape[1]):
            psi.append((new_dihedral(paths_forward_[i, j, angle_1, :])))
            phi.append((new_dihedral(paths_forward_[i, j, angle_2, :])))
        tmp = ax.plot(phi, psi, marker='o', color='b', markersize=2, alpha=1.)
        # linestyle='None', 
        colors.append(tmp[0].get_color())
    # ax.scatter(phis_start, psis_start, edgecolors='black', c='w', zorder=100, s=100, marker='*')

    # target = target.squeeze().detach().cpu().numpy()  # .view(-1, 3)
    psis_target = []
    phis_target = []
    ...
    plt.xlabel('phi')
    plt.ylabel('psi')
    plt.savefig(f"{filename}_forward.png")
    plt.show()
    plt.clf()
    ...
    plt.contourf(xs, ys, z, levels=100, zorder=0)
    tmp = ax.plot(phi, psi, marker='o', color='b', linestyle='None', markersize=2, alpha=1.)
    ...
    for i in range(0, paths_.shape[0]):
        ...
        tmp = ax.plot(phi, psi, marker='o', color='r', markersize=2, alpha=1.)
    ...
    plt.xlabel('phi')
    plt.ylabel('psi')
    plt.savefig(f"{filename}_learned.png")
    plt.show()
    plt.clf()
    ...
    plt.contourf(xs, ys, z, levels=100, zorder=0)
    tmp = ax.plot(phi, psi, marker='o', color='r', linestyle='None', markersize=2, alpha=1.)
    ax.scatter(phis_start, psis_start, edgecolors='black', c='w', zorder=100, s=100, marker='*')
    ax.scatter(phis_target, psis_target, edgecolors='w', c='w', zorder=100, s=10)
    plt.savefig(f"{filename}_learned_point.png")
    plt.clf()
    ...
    if paths_linear is not None:
        ...
        # print (paths_linear.shape)
        paths_linear_ = paths_linear_[0:1]

        for i in range(0, paths_linear_.shape[0]):
            psis_start.append((new_dihedral(paths_linear_[i, 0, angle_1, :])))
            phis_start.append(new_dihedral(paths_linear_[i, 0, angle_2, :]))

            psi = []
            phi = []
            for j in range(0, paths_linear_.shape[1]):
                psi.append((new_dihedral(paths_linear_[i, j, angle_1, :])))
                phi.append((new_dihedral(paths_linear_[i, j, angle_2, :])))
            tmp = ax.plot(phi, psi, marker='o', color='r', markersize=2, alpha=1.)
